...
RECIPE_PROMPT = "a wiry station engineer at the rail"


def recipe_for(c):
    return {"look_hash": look_hash(c), "builder_version": "builder/1", "fields_used": ["prompt_safe_description"],
            "rendered_sha256": hashlib.sha256(RECIPE_PROMPT.encode()).hexdigest()}


def candidate(root, c, seed, **kw):
    """A candidate generated from the packet's recipe: receipted with the look_ref, the recipe and the rendered prompt."""
    return image(root, seed, look_refs=look_refs_for(c), receipt_prompt_recipe=recipe_for(c), prompt=RECIPE_PROMPT, **kw)


def _pending_checkpoint(root, c, candidates, *, status="awaiting_human", recipe=None, look_ref=None):
    from lib.look_ingest import active_looks

    rec = active_looks(root)[("character", CHAR)]
    entry = {"entity_kind": "character", "entity_id": CHAR,
             "look_ref": look_ref or {"entity_kind": "character", "entity_id": CHAR, "look_hash": rec.look_hash, "receipt_id": rec.receipt_id},
             "prompt_recipe": recipe or recipe_for(c), "candidates": candidates}
    cp = {"version": "1.0", "project_id": "p", "pipeline_type": "authored-film", "stage": "headshots",
          "status": status, "timestamp": "2026-08-26T00:00:00+00:00",
          "pipeline": pinned_pipeline(root, "authored-film").to_dict(), "predecessors": [],
          "artifacts": {"headshot_packet": {"version": "1.0", "state": "pending", "characters": [entry]}}}
...
class TestSelectionGate:
    def test_selection_constructs_signed_record_from_packet(self, root, human_tty):
        ...
        cands = [candidate(root, c, f"c{i}") for i in range(3)]
    ...
    def test_main_prompts_for_candidate(self, root, human_tty, monkeypatch, capsys):
        c = character_look()
        activate_look(root, c)
        _pending_checkpoint(root, c, [candidate(root, c, "c0"), candidate(root, c, "c1")])
        req = _headshot_request(root)
        answers = iter(["2", "y", ""])
        ...
        assert "record to be signed" in out and '"candidates_checkpoint_digest"' in out


class TestNewKinds:
    def test_look_lock_request_with_envelope(self, root, human_tty, tmp_path):
        from lib.look_ingest import active_looks, look_lock_request, parse_look_ticket

        wf = tmp_path / "wayfinder-project"
        ticket = write_ticket(wf, character_look())
