...
_Drafted by Claude for Codex review, 2026-08-28. Follows the claudex-loop Phase 2 protocol; MAX_ROUNDS=5. Revision 3 after Codex rounds 1–2 (see REVIEW-LOG)._
...
- **Grandfather** (Bloodless/Ace): `scripts/headshot_run.py --project <slug> --entity <id> --grandfather` judges the active legacy hero under bundle 1.1 (an attempt in the hero budget, `role: hero`, imported/generated sentinel per its receipt), then writes a `headshot_grandfather` gate request; the human signs a receipt whose record binds `{entity_id, legacy_headshot_receipt_id, asset_id, look_hash, qc_receipt_id}` (new approval kind, envelope `{supersedes_receipt_id: legacy}`). `verify_active_headshot` accepts a 1.0 record only when such a receipt is the chain tip for the entity. The migration request to 1.4 is refused while any active hero of a cast character lacks one (`prepare_migration_request` gains this precondition for 1.4).
