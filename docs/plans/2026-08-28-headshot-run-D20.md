...
_Drafted by Claude for Codex review, 2026-08-28. Follows the claudex-loop Phase 2 protocol; MAX_ROUNDS=5. Revision 4 after Codex rounds 1–3 (see REVIEW-LOG)._
...
- **Policy bundles are versioned executables, not one mutable dict** (R2#1): `lib/sheet_qc/policy.py` keeps `BUNDLES = {"1.0": <frozen sheet-only bundle, byte-for-byte what 1.3 configs pinned>, "1.1": <1.0 + hero checklist, evidence kinds, hero local rules>}`; `bundle_sha256(version)`; the signed config names the version by its hash, and the judge/verifier load exactly that bundle. A 1.3 project keeps validating against 1.0 forever; a 1.4 project must pin 1.1. A test asserts the 1.0 hash equals the value Bloodless signed on 2026-08-27.
...
4. **Order of operations** (R3#3): persisted run state / a request in `done/` for this entity is resumed or finished FIRST (step 6, step 7 resume); only a genuinely new run is subject to the guard: if an active look exists for the key, refuse unless `--supersede`, in which case the request carries `supersedes_look_hash` (existing envelope field).
...
Evidence kinds gain `look_hair`, `look_age_band`, `look_build` (text from the signed active look). Local rules for `hero`: square or portrait, long edge ≥ 1024, no alpha. Bundle 1.1 is required ONLY for projects migrating to / running 1.4; 1.3 projects keep bundle 1.0 and nothing about them changes (R3#1). Golden tests pin bundle 1.0's hash and the byte-identity of the legacy director file.
...
- R2: `headshot_record` 1.1 changes the digest under 1.4; Ace's 1.3-era hero is accepted only through the signed grandfather receipt (D20.7), one more Bloodless signature (three in total for D20: config 1.2, grandfather for Ace, migration 1.4 — the migration is refused until the grandfather exists).
