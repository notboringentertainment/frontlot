...
_Drafted by Claude for Codex review, 2026-08-28. Follows the claudex-loop Phase 2 protocol; MAX_ROUNDS=5. Revision 5 after Codex rounds 1–4 (see REVIEW-LOG)._
...
- **project_config 1.2** = 1.1 + required `qc.max_hero_attempts` (int 1..24). Dual-read 1.0/1.1/1.2 in the loader; hero runs require 1.2 (no code default: the cap is signed or the command refuses). Bloodless re-signs once (this also carries `hero_policy_sha256`, D20.3).
...
1. Versions: authored-film 1.4 manifest (own director path); versioned policy bundles 1.0 (frozen, hash-pinned test) / 1.1; project_config 1.2 (dual-read 1.0/1.1/1.2); headshot_packet 1.1 (dual-read); headshot record 1.1; `headshot_grandfather` approval kind; `import_record` entity binding; tests.
