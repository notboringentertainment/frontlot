...
| Extractor atom index | `atoms/atoms.jsonl` (one JSON object per line: `id`, `type` FACT/RULE/CHARACTER/TIMELINE/KNOWLEDGE/OPEN/CONFLICT, `claim`, `entities`, `source{file,lines,anchor}`, `authority_rank`, `canon_status`, `spoiler`) + `atoms/ATOMS.md` index; sources extracted to `atoms/txt/` | Map by `canon_status`: `canon-ratified` → lock (evidence_ref = `id` + `source`); `canon-shape` → lock on direction only, wording stays open; `open` / `conflict` → open_questions[]; `unratified-input`, `superseded`, `governance` → NEVER locks. `authority_rank` orders collisions (1 = the project's Canon Note, highest). **`spoiler: true` atoms are excluded from any trailer/teaser/pitch output** — carry them into the packet as locks tagged `spoiler` so script and scene directors can refuse them, never into a trailer beat. A project Canon Note (`<Project> Canon Note.md`, statuses RATIFIED/SHAPE/OPEN) is the human-readable master of the same rulings; quote it for lock wording. |
