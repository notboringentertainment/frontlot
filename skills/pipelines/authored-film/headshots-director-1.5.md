...
## 1.5 — field-bound batch overrides (what changed from 1.4)

Under authored-film 1.5 the exhausted-budget flow separates the two decisions
1.4 collapsed:

1. **Waiver gate first.** When the hero budget is spent and failed candidates
   could be unlocked, headshot_run raises ONE `qc_override` request carrying
   the whole reviewed field (every failed candidate, its image, its failing
   items). The signer reconstructs that field from the QC chain, previews
   exactly which faces each accepted item unlocks, and the signed record 1.1
   binds the field digest, the previewed unlock list, the look (hash AND
   receipt), the config snapshot, and the request id. It fires even when some
   candidates already pass. Declining requires a typed note.
2. **Casting gate second.** The selection packet (headshot_packet 1.2)
   presents every passing-or-unlocked candidate — up to the signed
   `qc.max_hero_attempts`, not 4 — each carrying its exact override citation.
   The writer casts; the headshot record 1.2 seals the citation.

Unchanged from 1.4: imports and grandfathering keep verdict-bound 1.0
overrides only; casting finality (unchosen candidates are rejected forever);
no agent ever signs or authors an approval record.
