#!/usr/bin/env python3
"""Probe A: stylized (non-photoreal) character through Seedream + Seedance 2.5. Spends real money.
...
def main():
    ...
    hero = run("seedream_image", {"operation": "text_to_image", "prompt": "graphic-novel illustration, bold ink lines and flat cel shading, limited palette of navy, ochre and cream: a weathered lighthouse keeper in a wool coat, front view, plain cream background. Clearly a drawing, not a photograph.", "image_size": "auto_1K", "num_images": 1, "output_format": "png", "output_dir": str(out), "project_dir": str(PROJECT)})
    view = run("seedream_image", {"operation": "edit", "prompt": "same illustrated character, same ink-and-cel style and palette, three-quarter view, same coat, plain cream background. Clearly a drawing, not a photograph.", "reference_image_paths": [hero.output_path], "image_size": "auto_1K", "num_images": 1, "output_format": "png", "output_dir": str(out), "project_dir": str(PROJECT)})
    def ref(path, role):
        return {"asset_id": Path(path).stem, "path": str(path), "role": role, "visual_bible_entity_id": "character-smoke"}
    clip = run("seedance_video", {"operation": "reference_to_video", "model_version": "2.5", "prompt": "Animated in the same ink-and-cel illustration style as @Image1 and @Image2, the same drawn lighthouse keeper turns his head slowly toward the camera, wind in his hair, 4 seconds, no dialogue.", "reference_image_paths": [hero.output_path, view.output_path], "reference_manifest": [ref(hero.output_path, "hero"), ref(view.output_path, "three_quarter")], "resolution": "480p", "duration": "4", "aspect_ratio": "16:9", "generate_audio": False, "output_path": str(out / "probe_a_stylized.mp4"), "project_dir": str(PROJECT)})
